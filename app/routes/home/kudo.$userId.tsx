...
import { SelectBox, type SelectBoxOption } from "~/components/select-box";
...
export const action: ActionFunction = async ({ request }) => {
	...
	const textColor = form.get("textColor") as string | null;
	const emoji = form.get("emoji") as string | null;
	const recipientId = form.get("recipientId") as string | null;

	const formErrorMsg = "Invalid Form Data";

	invariant(message, formErrorMsg);
	invariant(recipientId, formErrorMsg);
	invariant(backgroundColor, formErrorMsg);
	invariant(textColor, formErrorMsg);
	invariant(emoji, formErrorMsg);
	...
	invariant(message.length, "Please provide a message.");
	invariant(recipientId.length, "No recipient found...");
...
export default function KudoModal() {
	...
	const handleChange = ({ target }: InputChangeEvent, field: string) => {
		setFormData((data) => ({ ...data, [field]: target.value }));
	};

	const { recipient, user } = useLoaderData();

	const handleStyleChange = ({ target }: InputChangeEvent, field: string) => {
		setFormData((data) => ({
			...data,
			style: {
				...data.style,
	...
	const getOptions = (data: Record<string, string>) => {
		// return Object.keys(data).reduce((acc: SelectBoxOption[], curr) => {
		// 	acc.push({
